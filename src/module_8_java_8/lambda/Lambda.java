package module_8_java_8.lambda;

import module_8_java_8.Student;

import java.util.Arrays;
import java.util.Collections;
import java.util.List;

public class Lambda {
    static List<Student> list = Arrays.asList(
            new Student("Nguyễn Văn A", true, 7.5),
            new Student("Trần Thị Thu B", false, 5.5),
            new Student("Phạm Đức C", true, 9.5),
            new Student("Lê Thị Mỹ D", false, 6.5),
            new Student("Đoàn Thị E", false, 8.0)
    );

    public static void main(String[] args) {
        demo1();
        demo2();
        demo3();
        demo4();
    }

    private static void demo4() {
        Demo4Inter o = (x) -> System.out.println(x);
        o.m1(1999);
    }

    private static void demo3() {
        Collections.sort(list, (sv1, sv2) -> -sv1.getMarks().compareTo(sv2.getMarks()));
        list.forEach(sv -> {
            System.out.println(">> Name : " + sv.getName());
            System.out.println(">> Marks: " + sv.getMarks());
            System.out.println();
        });
        System.out.println("//////////////////////////////////////");
    }

    private static void demo2() {
        list.forEach(sv -> {
            System.out.println(">> Name : " + sv.getName());
            System.out.println(">> Marks: " + sv.getMarks());
            System.out.println();
        });
        System.out.println("//////////////////////////////////////");
    }

    private static void demo1() {
        List<Integer> list = Arrays.asList(1, 3, 11, 4, 7, 2);
        list.forEach(n -> System.out.println(n));
        System.out.println("//////////////////////////////////////");
    }

    @FunctionalInterface
    interface Demo4Inter {
        void m1(int x);

        default void m2() {
        }

        public static void m3() {

        }
    }
}
